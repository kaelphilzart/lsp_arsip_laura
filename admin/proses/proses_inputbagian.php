<?php
	...
	if (!($nama_bagian=='') and !($username_admin_bagian=='') and !($password_bagian =='') and !($nama_lengkap=='') and !($no_hp_bagian=='')){		
		
		$nama_baru = $username_admin_bagian. $ext_file;
		$path = "../../bagian/images/".$nama_baru;
		move_uploaded_file($tmp_file, $path);
		
		$sql = "INSERT INTO tb_bagian(nama_bagian, username_admin_bagian, password_bagian, nama_lengkap, no_hp_bagian, gambar)
				values ('$nama_bagian', '$username_admin_bagian', '$password_bagian', '$nama_lengkap', '$no_hp_bagian', '$nama_baru')";
		$execute = mysqli_query($db, $sql);
		
		echo "<Center><h2><br>Terima Kasih<br>Account Bagian Telah Didaftarkan ke Sistem</h2></center>
