<?php
    ...
    if (!($tgl_masuk=='') and !($kode_suratmasuk =='') and !($nomorurut_suratmasuk  =='') and !($nomor_suratmasuk =='') and !($tgl_surat =='') and !($pengirim =='')  and !($perihal_suratmasuk =='') and !($operator =='') and !($tanggal_entry =='') and  ($tipe_file == "application/pdf") and ($ukuran_file <= 10340000)){		
		
		$nama_baru = $thnNow.'-'.$nomorurut_suratmasuk . $ext_file;
		$path = "../surat_masuk/".$nama_baru;
		move_uploaded_file($tmp_file, $path);
		
		$sql = "INSERT INTO tb_suratmasuk(tanggalmasuk_suratmasuk, kode_suratmasuk, nomorurut_suratmasuk, nomor_suratmasuk, tanggalsurat_suratmasuk, pengirim, perihal_suratmasuk, file_suratmasuk, operator, tanggal_entry )
				values ('$tgl_masuk', '$kode_suratmasuk', '$nomorurut_suratmasuk ', '$nomor_suratmasuk', '$tgl_surat', '$pengirim', '$perihal_suratmasuk', '$nama_baru', '$operator', '$tanggal_entry')";
